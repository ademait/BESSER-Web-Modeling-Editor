...
class CanvasElementComponent extends Component {
    render() {
        const { hovered, selected, remoteSelectors, moving, interactive, interactable, element, child: ChildComponent, children, theme, zoomFactor: _zoomFactor, selectionBoxActive: _selectionBoxActive, allElements, id: _id, type: _type, name: _name, ...props } = this.props;
        ...
        if (UMLContainer.isUMLContainer(element) && ChildComponent) {
            elements = element.ownedElements
                ...
                .map((id) => React.createElement(ChildComponent, { key: id, id: id }));
        }
