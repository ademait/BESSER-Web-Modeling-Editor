import React from 'react';
import { Button } from './style-pane-styles';
import styled from 'styled-components';
const colors = [
    '#fc5c65',
    '#fd9644',
    '#fed330',
    '#26de81',
    '#2bcbba',
    '#45aaf2',
    '#4b7bec',
    '#6a89cc',
    '#a55eea',
    '#d1d8e0',
    '#778ca3',
    'black',
];
const ColorContainer = styled.div `
  display: flex;
  flex-direction: column;
  align-items: center;
  background-color: ${(props) => props.theme.color.background};
  width: 100%;
  padding-bottom: 10px;
`;
const Flex = styled.div `
  display: flex;
  justify-content: center;
  align-items: center;
  flex-wrap: wrap;
`;
const Color = styled.button.attrs({}) `
  height: 28px;
  width: 28px;
  background-color: ${({ color }) => color || 'black'};
  border-radius: 14px;
  cursor: pointer;
  border: none;
  position: relative;
  margin: 10px;
  box-shadow: ${({ color, selected }) => (selected ? `0px 0px 10px ${color}` : 'none')};
`;
export function ColorSelector({ onColorChange, color, open }) {
    const handleColorChange = (newColor) => {
        onColorChange(newColor);
    };
    const reset = () => {
        onColorChange(undefined);
    };
    if (!open)
        return null;
    return (React.createElement(React.Fragment, null, open ? (React.createElement(ColorContainer, null,
        React.createElement(Flex, null, colors.map((colorOption) => (React.createElement(Color, { key: colorOption, color: colorOption, onClick: () => handleColorChange(colorOption), selected: colorOption === color })))),
        React.createElement(Button, { onClick: reset }, "Reset"))) : null));
}
//# sourceMappingURL=color-selector.js.map