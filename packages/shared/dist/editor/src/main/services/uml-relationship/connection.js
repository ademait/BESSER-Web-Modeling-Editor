...
export class Connection {
    static computePath(source, target, options) {
        ...
        const sourcePorts = source.element.type in UMLRelationshipType
            ? getPortsForRelationship(source.element)
            : getPortsForElement(source.element);
        const targetPorts = target.element.type in UMLRelationshipType
            ? getPortsForRelationship(target.element)
            : getPortsForElement(target.element);
        const sourcePortPosition = sourcePorts[source.direction].add(source.element.bounds.x, source.element.bounds.y);
        const targetPortPosition = targetPorts[target.direction].add(target.element.bounds.x, target.element.bounds.y);
