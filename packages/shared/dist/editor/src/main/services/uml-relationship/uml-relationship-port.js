...
export const RELATIONSHIP_TYPES_WITH_CENTER_PORT = [
    'ClassBidirectional',
    'ClassUnidirectional'
];
/**
 ...
 */
export function canHaveCenterPort(relationship) {
 ...
 */
export function getRelationshipCenterPoint(relationship) {
    if (!relationship || !relationship.path || relationship.path.length < 2) {
        return new Point(relationship?.bounds?.width ? relationship.bounds.width / 2 : 0, relationship?.bounds?.height ? relationship.bounds.height / 2 : 0);
    }
    ...
    if (middleIndex === 0) {
        ...
        return new Point(startPoint.x + (endPoint.x - startPoint.x) / 2, startPoint.y + (endPoint.y - startPoint.y) / 2);
    }
 ...
 */
export function getPortsForRelationship(relationship) {
...
//# sourceMappingURL=uml-relationship-port.js.map