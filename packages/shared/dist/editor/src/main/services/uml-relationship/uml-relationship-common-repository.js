...
export const UMLRelationshipCommonRepository = {
    ...
    getSupportedConnectionsForElements: (elements) => {
        ...
        let initialSupportedConnections;
        if (UMLRelationship.isUMLRelationship(elementsArray[0])) {
            ...
            initialSupportedConnections = elementsArray[0].constructor.supportedRelationships || [];
        }
        else {
            // For regular elements, get the supported relationships from UMLElements
            initialSupportedConnections = UMLElements[elementsArray[0].type].supportedRelationships;
        }
        ...
        return elementsArray.reduce((supportedConnections, element) => {
            ...
            if (UMLRelationship.isUMLRelationship(element)) {
                // For relationships, we need to get the supported relationships from the element itself
                const elementSupportedConnections = element.constructor.supportedRelationships || [];
