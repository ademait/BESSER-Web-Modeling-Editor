...
export const computeMiddlePositionForUMLAssociation = (alignmentPath) => {
    if (alignmentPath.length < 2)
        return new Point();
...
export const UMLAssociationComponent = ({ element }) => {
    ...
    const middle = computeMiddlePositionForUMLAssociation(path);
    ...
    return (React.createElement("g", null,
        ...
        showAssociationNames && element.name && !isInheritance && !isLinkRel && (React.createElement("text", { x: middle.x || 0, y: middle.y || 0, textAnchor: "middle", dy: "-5", pointerEvents: "none", style: { ...textFill, fontSize: '12px', fontWeight: 'bold' } }, element.name)),
        !isInheritance && !isLinkRel && (React.createElement(React.Fragment, null,
